...
library(pacman)
pacman::p_load(tidyverse, readxl, sqldf, ggpubr, arules, arulesViz, dplyr, stringr)


```


```{r clear-objects-prior-to-cleaning, include = FALSE}
...
```{r load-raw-data, include=FALSE, warning = FALSE}
...
```{r rewrite-column-names, include=FALSE}
...
```{r re-classify-income-categories, include = FALSE}
...
```{r recode-age-data, include = FALSE}
...
```{r clean-age-data, include = FALSE}
...
```{r repair-spelling, include = FALSE}
...
```{r update-single-laundry-events, include = FALSE}
...
```{r recode-over-80s, include=FALSE}
...
```{r recode--0-child-counts, include=FALSE}
...
```{r recode-child<0, include=FALSE}
...
```{r recode-child-ages, include=FALSE}
...
#$Grocery_Other <-  str_to_lower(diary$Grocery_Other) #move all to lowercase to aid regexes
...
```{r select-unique-demographics, include = FALSE}
...
```{r prepare-sql-selection-chunk, include=FALSE}
...
```{r write-cleaned-files, include = FALSE}
...
```{r clear-envrionment-for-analysis-block, include = FALSE}
...
#demo_original <- read.csv("demographics.csv")  #Commenting out, I don't think we use this anymore. 
...
```{r prepare-segmentation-data, echo = FALSE}
segment <- demo %>% 
  group_by(segment) %>%
    summarize(Count = n()) %>%
...
```{r check-engagement-time, echo = FALSE, include = FALSE}
...
As segmentation was a new feature with recent data, NA's, or uncategoried respondents, are present and represent 37% of the entries. The NA's were kept as a reminder that the segmentation data is only partially representative of the entire sample, and to maintain size context when evaluating differences between segments. 
...
segment %>% 
 ...
 # arrange(Count) %>%
  ...
  coord_flip() +
  labs( x= "", y = "Count of Respondents", title = "Segment Population", subtitle = "NAs and Mellow Methodicals Dominate") +
   theme_minimal() 
...
```{r filter-segment, include = FALSE}
...
```{r plot-segment-child, include = FALSE}
...
```{r plot-segment-income, echo = FALSE, warning=FALSE}
...
```{r plot-fHOH-educaction, include = FALSE}
demo %>% 
...
```{r plot-mHOH-education, include = FALSE}
demo %>% 
...
```{r tabulate-frequency, include = FALSE}
...
```{r table-event-counts}
table(diary$Event_Count_Today)

```


```{r establish-events-df, echo = FALSE}
...
```{r segment-day-volume, echo = FALSE}
Events_Demo %>% 
...
```{r respondent-is-HOH, echo = FALSE}
...
Having Establihsed the respondent age as a proxy for the HOH, or as an age representative of the driver of the laundry activity, we review the age distribtuion of the demographics. 
Mean and median age are both 42. The Interquartile range o the sample, or the middle 50 percent, is 40 to 65 years old. This is not representative of the United States, which reports a median age of 39. (census.gov)

Please Note the spike of respondents at aage 89, this is a data cleaning artifact, as "80+" was re coded as 89 years old. The sample likely tapers off like the younger end of the distribution. 

```{r demo-age-stats, include = FALSE}
mean(demo$respondent_age)
median(demo$respondent_age)
min(demo$respondent_age)
summary(demo$respondent_age)

```


```{r plot-age-distro, echo = FALSE}
demo %>%
  ggplot(aes(x = respondent_age)) +
  geom_histogram(binwidth = 1, fill = "#1e9fc7") +
  scale_x_continuous(breaks = seq(15, 90, by = 5)) +
  geom_vline(xintercept =  c(40, 52, 65), color = "lightblue") +
  labs(title = "Multimodal Age Distibution", subtitle = "Mean and Median - 52 | IQR 40-65", x = "Respondent Age", y = "")
```
...
```{r under-40s-marriage, echo = FALSE}
...
```{r under-40s-own-rent, echo = FALSE}
...
```{r plot-under-40s-own-rent, echo = FALSE}
...
```{r under40s-children, echo = FALSE}
...
```{r laundry-events-1, include  = FALSE}
...
```{r children-households, echo = FALSE}
...
```{r plot-children-freq, echo = FALSE}
events_children %>%
  ggplot(aes(x = Prop_Days)) +
  geom_histogram(binwidth = .05, fill = "#1e9fc7") +
...
```{r plot-no-children-freq, echo = FALSE}
events_no_children %>%
  ggplot(aes(x = Prop_Days)) +
  geom_histogram(binwidth = .05, fill = "#1e9fc7") +
...
```{r remove-objects-for-MB, include = FALSE}
#Remove items for next section
rm(events_children, events_no_children)
...
Market Basket Analysis (MBA) is a data mining technique that can help understand the relationships between items that consumers purchase. The analysis is based on the assumption that customers who buy a certain group of items are likely to be interested in other items that are often bought in the same transaction. By analyzing customer purchase data, we  can identify these associations (association rules) and use them to inform potential marketing strategies that focus on increasing sales and customer lotalty.

MBA's are typically created from transaction data, whether from brick and mortar or e-commerce retailers. In this situation however, we must infer consumers' transactions based on the products they report using in their day-to-day laundry.
```{r start-MB, include = FALSE}
...
# Commenting out for combining the script
...
```{r remove-colnames, include = FALSE}
...
```{r explort-MB-cleaned, include = FALSE}
...
```{r import-MB-cleaned, include = FALSE}
...
```{r plot-MB-top-20-prop, echo = FALSE}
...
### Interpreting a Market Basket Analysis: A Simple Overview

In the Output of an MBA in R, there is a Left-Hand Side (LHS) and a Right-Hand Side (RHS). These can easily be interpreted as the bundle of items in a customer's cart (LHS), and the additional item (RHS) that is frequently purchased with the LHS bundle. In a classic Amazon / E-commerce example, the RHS would be the item recommended as "people also frequently purchased...".

There are also statistical metrics to take note of:

**Support**, **confidence**, and **lift** are all measures that are used in market basket analysis to identify the strength of the association rules between different items in a dataset.

* **Support** is the percentage of transactions in which a particular item or groups of items appear. For example, if an item appears in 10% of all transactions, its support would be 10%.

* **Confidence** is a measure of the strength of the association between two items or bundles. It is calculated as the percentage of transactions that contain both items, divided by the percentage of transactions that contain the first item. For example, if items in the LHS appear in 50% of transactions and the RHS item appears in 40% of transactions, and both appear together in 30% of transactions, the confidence of the association between LHS and RHS would be 60% (30% / 50%).

* **Lift** is a measure of the strength of the association between the LHS and RHS relative to their individual support. It is calculated as the confidence of the association divided by the support of the RHS item For example, if the confidence of the association between LHS and RHS is 60% and the support of RHS is 40%, the lift of the association between LHS and RHS would be 1.5 (60% / 40%). A lift value greater than 1 indicates a strong association between the two items, while a value less than 1 indicates a weak association. Lift can also be interpreted as "how much our confidence has increased that RHS item will be purchased given that the RHS was purchased, in this case we are now 1.5 times more confident. 

### Check to see how many association rules the current settings have generated
```{r establish-MB-rules, echo = FALSE}
#Creating the association rules based on parameters.
# to make rules stricter, increase supp (support) and conf (confidence) measures.
...
```{r inpsect-MB-rules-top-5, echo = FALSE}
...
```{r remove-MB-objects, include = FALSE}
rm(rules, MB_Data)#Remove items to prepare for the next section
...
Simple keyword matching was performed to capture counts and sample text entered related to respondent frustrations. Distinct categories were selected based on a sampling of some of the comments reviewed. Categories include frustrations related to People, Chores, Soils, and those without frustrations. 

Simple Keyword selection is a crude analysis tool. Counting entries where keywords exists will not capture context, sarcasm, mis-spellings, double-negatives or other grammatical idiosyncrasies. 

A random sample from each category is presented to provide the "flavor" of each complaint. Note the re-execution of the script will result in a new random sample. 

```{r prep-frustrations, include = FALSE}
# Prepare dataset for text analysis
#Subset Frustrations Data
frustrations <- diary %>% #Subset the frustrations Entry
  select('respondent_id', 'Frustrations')
...
tips <- diary %>% #Subset the frustrations Entry
  select('respondent_id', 'Tips_Tricks')
...
text <- frustrations %>% 
  select_all() %>%
  full_join(tips, by = "respondent_id")

write.csv(text, "text.csv", row.names = FALSE)

text <- read.csv("text.csv") #Load in the data
```


```{r remove-temp-dfs, include = FALSE}
rm(tips, frustrations) #Remove redundancies
```


```{r get-uniques-, include = FALSE}
...
households <- length(unique(text$respondent_id))  #6685 Households reporting

```


## Frustrations Related to Other People 

219 Comments related to other people, selected to capture any entries with the following keywords: 

"husband", "son", "wife", "daughter", "mother", "father", "sister", "brother", "spouse", "sibling", "kid", "kids", "shar.", "people"

```{r people-frustrations, echo=FALSE}
...
## Random Sample - 10 People Frustrations


```{r sample-people-frustrations, echo = FALSE}
sample(people_frustrations, 10)
```



## Frustrations Related to Specific Tasks 

1394 Comments related to task/chores, too many to print below. 745 (53%) include complaints including the word "time", which may mean that half the pain of laundry is the time it takse from the respondents day. 

Keywords or this selection are: 
"pockets", "residue", "time",  "long", "bend.", "shrink", "wrinkle", "lint", "chore", "sort", "hang", "rewash", "broken", "static"

```{r task-frustrations, include  = FALSE}
...
length(task_frustration)


length(str_subset(frustrations, "time"))

```
## Random Sample - Task Frustrations 

```{r, task-sample, echo = FALSE}
sample(task_frustration, 10)

```

## Frustrations - Specific Soils

308 Frustrations Linked to Soils Keywords
"dirty", "smell", "dingy", "oil", "grease", "dirt", "blood", "grass"

```{r soil-frustrations, include = FALSE}
...
#soils_frustration 
```
## Random Sample - Soils Frustrations

```{r sample-soils, echo = FALSE}
sample(soils_frustration, 10)
```

## Economic Frustration


```{r economics, include = FALSE}
#Economic considerations

cost <- c("cost", "price", "expensive", "money", "coin.", "quarter.", "inflation", "bills", "dollar") 
cost_match <- str_c(cost, collapse = "|")

cost_frustration <- str_subset(frustrations, cost_match) 

length(cost_frustration)



```

## Random Sample - Economic Frustrations

```{r sample-economics, echo = FALSE}
sample(cost_frustration, 25)
```


## No Frustration - Laundry Enthusiasm

288 Comments including keywords like: "like", "love", "enjoy", "fun". 
Numerous negatives captured here. See the sampling below. 


```{r enthusiasm, include = FALSE}
#Who likes doing laundry?
likes <- c("like", "love", "enjoy", "^fun$", "relax") #the string "like" is too ambiguous to use siomple here
likes_match <- str_c(likes, collapse = "|")

likes_frustration <- str_subset(frustrations, likes_match) 

length(likes_frustration) 

```
## Random Sample - Laundry Enthusiasm

```{r sample-enthusiasm, echo = FALSE}
sample(likes_frustration, 25)
```
# Tips+Tricks Review

3336 Entries were present in the tips and tricks data.
...
```{r no-tips, include = FALSE}
#How many nos?
nos <- c("no", "none", "not", "no.", "NA") #
nos_match <- str_c(nos, collapse = "|")

no_tips <- str_subset(tips, nos_match) 

```


## Additive Tips and Tricks 

216 Entries Mention an additive from the list, 108 of those mention vinegar. 68 mention "soda", and 40 mention "oxi" whether mentioned in Oxi-Clean or hydrogen peroxide. 

```{r tips-additives, include = FALSE}

chems <- c("vinegar", "lemon", "alcohol", "soda", "oxi", "acid", "citric", "ammonia", "bleach", "hydrogen") #
chems_match <- str_c(chems, collapse = "|")

chem_tips <- str_subset(tips, chems_match)

length(chem_tips)

length(str_subset(tips, "vinegar"))
length(str_subset(tips, "soda"))
length(str_subset(tips, "oxi"))


```

## Sample - Additives
```{r sample-additives, echo = FALSE}
...
## Folding and Handling Tips

397 Tips including keywords like "fold", "stack",  "hang", "basket", "carry", "store". 
128 of those tips remind us to prevent wrinkles by hanging or folding. 
```{r hanlde-tips, include = FALSE}

handle <- c("fold", "stack",  "hang", "basket", "carry", "store") #
handle_match <- str_c(handle, collapse = "|")

handle_tips <- str_subset(tips, handle_match)

length(handle_tips)

length(str_subset(handle_tips, "wrinkle"))

```

## Handling Sample

```{r sample-handling, echo=FALSE}

sample(handle_tips, 10)
